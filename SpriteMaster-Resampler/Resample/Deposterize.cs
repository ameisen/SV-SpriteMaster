...
internal static class Deposterize {
	...
	private ref struct DeposterizeContext {
		...
		private Color16 Merge(Color16 reference, Color16 lower, Color16 higher) {
			...
			bool doMerge;
			if (SMConfig.Resample.Deposterization.UsePerceptualColor && lower != higher && (lower == reference || higher == reference)) {
				doMerge =
	...
	*/
	...
	internal static Span<Color16> Enhance(
		...
		var context = new DeposterizeContext(
			size: size,
			wrapped: wrapped,
			passes: passes ?? SMConfig.Resample.Deposterization.Passes,
			threshold: threshold ?? SMConfig.Resample.Deposterization.Threshold,
			blockSize: blockSize ?? SMConfig.Resample.Deposterization.BlockSize,
			useRedmean: SMConfig.Resample.Deposterization.UseRedmean
		);
