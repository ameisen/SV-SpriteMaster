...
namespace Benchmarks.Hashing.Methods;

internal static unsafe class Marvin32Unsafe {
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static int ComputeHash32(ReadOnlySpan<byte> data, ulong seed) =>
		ComputeHash32((byte*)Unsafe.AsPointer(ref MemoryMarshal.GetReference(data)), (uint)data.Length, (uint)seed, (uint)(seed >> 32));

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static int ComputeHash32(ReadOnlySpan<byte> data) =>
		ComputeHash32((byte*)Unsafe.AsPointer(ref MemoryMarshal.GetReference(data)), (uint)data.Length, DefaultSeedHigh, DefaultSeedLow);

	private static int ComputeHash32(byte* data, uint count, uint p0, uint p1) {
		...
		do {
			...
			p0 += Unsafe.ReadUnaligned<uint>(data);
			uint nextUInt32 = Unsafe.ReadUnaligned<uint>(data + 4);
	...
	DoFinalPartialRead:
		...
		uint partialResult = Unsafe.ReadUnaligned<uint>((data + (count & 7)) - 4);
	...
	InputTooSmallToEnterMainLoop:
		...
		if ((count & 0b_0010) != 0) {
			...
			if (BitConverter.IsLittleEndian) {
				partialResult <<= 16;
				partialResult |= (uint)Unsafe.ReadUnaligned<ushort>(data);
			}
			else {
				partialResult |= (uint)Unsafe.ReadUnaligned<ushort>(data);
	...
	private const uint DefaultSeedHigh = 0x0000_1505U;
	private const uint DefaultSeedLow = 0x1505_1505U;
	private const ulong DefaultSeed = ((ulong)DefaultSeedHigh << 32) | DefaultSeedLow;
}
