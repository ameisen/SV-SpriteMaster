...
namespace SpriteMaster.Extensions {
	internal static class Floating {
		...
		[MethodImpl(Runtime.MethodImpl.Optimize)]
		internal static Vector2 Saturate (this Vector2 v) => new Vector2(v.X.Saturate(), v.Y.Saturate());

		[MethodImpl(Runtime.MethodImpl.Optimize)]
		internal static float Lerp (this float s, float x, float y) => x * (1.0f - s) + y * s;

		[MethodImpl(Runtime.MethodImpl.Optimize)]
		internal static float Lerp (this float s, float x, double y) => unchecked((float)(x * (1.0f - s) + y * s));
		...
		[MethodImpl(Runtime.MethodImpl.Optimize)]
		internal static Vector2 Lerp (this float s, Vector2 x, Vector2 y) => new Vector2(s.Lerp(x.X, y.X), s.Lerp(x.Y, y.Y));

		[MethodImpl(Runtime.MethodImpl.Optimize)]
		internal static Vector2 Lerp (this double s, Vector2 x, Vector2 y) => new Vector2((float)s.Lerp((double)x.X, (double)y.X), (float)s.Lerp((double)x.Y, (double)y.Y));

		[MethodImpl(Runtime.MethodImpl.Optimize)]
		internal static Vector2 Lerp (this Vector2 s, Vector2 x, Vector2 y) => new Vector2(s.X.Lerp(x.X, y.X), s.Y.Lerp(x.Y, y.Y));

		[MethodImpl(Runtime.MethodImpl.Optimize)]
		internal static Vector2 Swap (this Vector2 v) => new Vector2(v.Y, v.X);
	}
}
