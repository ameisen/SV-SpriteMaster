...
internal static class Marvin32 {
	...
	public static int ComputeHash32(ReadOnlySpan<byte> data) =>
		ComputeHash32(ref MemoryMarshal.GetReference(data), (uint)data.Length, DefaultSeedHigh, DefaultSeedLow);
	...
	private static int ComputeHash32(ref byte data, uint count, uint p0, uint p1) {
		...
		do {
			...
			uint nextUInt32 = Unsafe.ReadUnaligned<uint>(ref Unsafe.AddByteOffset(ref data, new(4)));
			...
			data = ref Unsafe.AddByteOffset(ref data, new(8));
		} while (--loopCount > 0);
	...
	DoFinalPartialRead:
		...
		uint partialResult = Unsafe.ReadUnaligned<uint>(ref Unsafe.Add(ref Unsafe.AddByteOffset(ref data, new(count & 7)), -4));
	...
	InputTooSmallToEnterMainLoop:
		...
		if ((count & 0b_0001) != 0) {
			...
			partialResult = Unsafe.AddByteOffset(ref data, new(count & 2));
