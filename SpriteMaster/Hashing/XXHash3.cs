...
partial class Hashing {
	...
	internal static unsafe class XXHash3 {
		...
		static unsafe XXHash3() {
			...
			XXH3_SECRET_ARRAY = GC.AllocateUninitializedArray<byte>(length: secret.Length, pinned: true);
			secret.CopyTo(XXH3_SECRET_ARRAY);
			fixed (byte* ptr = XXH3_SECRET_ARRAY) {
				SSH3_SECRET = ptr;
			}
		}
		...
		internal static ulong Hash64(ReadOnlySpan<byte> data, int length) {
			if (length <= 16) {
				...
				return xxh3_17to128_64(data, length, SSH3_SECRET);
			}
			else if (length <= 240) {
				return xxh3_129to240_64(data, length, SSH3_SECRET);
			}
			else {
				return xxh3_hashLong_64(data, length, SSH3_SECRET);
			}
		}
		...
		private static ulong xxh3_merge_accs(Span<ulong> acc, byte* secret, ulong start) {
			ulong result = start;
		...
		[MethodImpl(MethodImpl.Hot)]
		private static void xxh3_accumulate(Span<ulong> acc, ReadOnlySpan<byte> data, byte* secret, int stripeCount) {
