...
internal static unsafe partial class XxHash3 {
	...
	[MethodImpl(Inline)]
	private static void Accumulate512Sse2(ulong* accumulatorStore, byte* data, byte* secret) {
	...
	[MethodImpl(Inline)]
	private static void ScrambleAccumulatorSse2(ulong* accumulatorStore, byte* secret) {
		PrefetchNonTemporalNext(secret);
	...
	[Pure]
	[MethodImpl(Inline)]
	private static ulong HashLongSse2(byte* data, uint length) {
		byte* secret = Secret;
