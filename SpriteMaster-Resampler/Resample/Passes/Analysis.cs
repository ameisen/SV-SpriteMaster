...
internal static partial class Analysis {
	...
	internal static unsafe LegacyResults AnalyzeLegacy(ReadOnlySpan<Color8> data, Bounds bounds, Vector2B wrapped, bool strict = true) {
...
#if SM_LIBRARY
			Console.Error.WriteLine("Inverted Sprite Bounds Value leaked to AnalyzeLegacy: {0}", bounds);
#else
		...
		if (SMConfig.Resample.Analysis.WrapDetection.Enabled) {
			float edgeThreshold = SMConfig.Resample.Analysis.WrapDetection.EdgeThreshold;
			...
			byte alphaThreshold = SMConfig.Resample.Analysis.WrapDetection.AlphaThreshold;
		...
		// Horizontal
		{
			for (int y = bounds.Top; gradientAxial.X && y < bounds.Bottom; ++y) {
				...
				for (int x = 1; x < bounds.Width; ++x) {
					...
					if (difference >= SMConfig.Resample.Analysis.MaxGradientColorDifference) {
	...
	[MethodImpl(Runtime.MethodImpl.Inline)]
	internal static LegacyResults AnalyzeLegacy(ReadOnlySpan<Color8> data, Bounds bounds, Vector2B wrapped) {
