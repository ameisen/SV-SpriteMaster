...
static class CgMath {
	...
	internal static Float3 MatrixMul(in Float3 vec, in Float3 m0, in Float3 m1, in Float3 m2) => (
		vec.X * m0.X + vec.Y * m1.X + vec.Z * m2.X,
		vec.X * m0.Y + vec.Y * m1.Y + vec.Z * m2.Y,
		vec.X * m0.Z + vec.Y * m1.Z + vec.Z * m2.Z
	);

	internal static Float3 MatrixMul(in Float4 vec, in Float3 m0, in Float3 m1, in Float3 m2, in Float3 m3) => (
	...
	internal static Float4 MatrixMul(in Float4 vec, in Float4 m0, in Float4 m1, in Float4 m2) => (
		vec.X * m0.X + vec.Y * m1.X + vec.Z * m2.X + vec.W * m2.X,
		vec.X * m0.Y + vec.Y * m1.Y + vec.Z * m2.Y + vec.W * m2.Y,
		vec.X * m0.Z + vec.Y * m1.Z + vec.Z * m2.Z + vec.W * m2.Z,
		vec.X * m0.W + vec.Y * m1.W + vec.Z * m2.W + vec.W * m2.W
	);

	internal static float Frac(float f) => f % 1.0f;

	internal static Float2 Frac(in Float2 vec) => (
		Frac(vec.X),
		Frac(vec.Y)
	);
	...
	internal static float RGBToYUV(in Float3 color) => color.Dot(Y);

	internal static float RGBToYUV(in Float4 color) => color.RGB.Dot(Y);
