...
namespace xBRZNet.Common
{
	public static class IMath
	{
		...
		private ref struct Argument<T> where T : unmanaged
		{
			...
			internal Argument(in T value, in string name)
			{
				Value = value;
				Name = name;
			}
		...
		[DebuggerStepThrough, DebuggerHidden(), MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static Argument<T> AsArg<T>(this T value, in string name) where T : unmanaged
		{
			return new Argument<T>(value, name);
		}

		[DebuggerStepThrough, DebuggerHidden(), MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static string GetCheckNotNegativeMessage<T>(in Argument<T> argument, in string memberName) where T : unmanaged
		{
		...
		private static void CheckNotNegative(
			...
			[CallerMemberName] in string memberName = null
		)
		{
			if (argument < 0)
			{
		...
		internal static int Sqrt(this in int value)
		{
		...
		// Truncating Square Root
		internal static uint Sqrt(this in uint value)
		{
			if (value >= SQRT_MIN && value < SQRT_MAX)
			{
		...
		// Truncating Square Root
		internal static long Sqrt(this in long value)
		{
			value.AsArg(nameof(value)).CheckNotNegative();
		...
		// Truncating Square Root
		internal static ulong Sqrt(this in ulong value)
		{
			if (value >= SQRT_MIN && value < SQRT_MAX)
			{
