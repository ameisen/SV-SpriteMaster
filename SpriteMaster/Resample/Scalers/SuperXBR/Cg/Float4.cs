...
partial struct Float4 {
	...
	internal Float4(float x, in Vector3 yzw) : this(new Vector4(x, yzw.X, yzw.Y, yzw.Z)) { }
	internal Float4(float x, in Float3 yzw) : this(new Vector4(x, yzw.X, yzw.Y, yzw.Z)) { }
	internal Float4(float x, float y, float z, float w) : this(new Vector4(x, y, z, w)) { }
	internal Float4(in (float X, float Y, float Z, float W) value) : this(value.X, value.Y, value.Z, value.W) { }

	internal readonly float Dot(in Float4 other) => Vector4.Dot(Value, other.Value);

	internal readonly Float4 Min(in Float4 other) => Vector4.Min(Value, other.Value);
