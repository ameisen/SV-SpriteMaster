...
public class Algorithms {
	...
	[MethodImpl(Hot)]
	private static U[] CopyToArray<T, U>(T[] data, int startIndex, int elementCount, out int typeSize) where T : struct where U : unmanaged {
		...
		var inDataCasted = MemoryMarshal.Cast<T, U>(inData);
		var resultData = GC.AllocateUninitializedArray<U>(inDataCasted.Length);
		inDataCasted.CopyTo(resultData);
		return resultData;
	}
