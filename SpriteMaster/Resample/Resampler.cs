...
internal sealed partial class Resampler {
	internal static Scaler CurrentConfiguredScaler =>
		Configuration.Preview.Override.Instance?.Scaler ?? SMConfig.Resample.Scaler;

	[StructLayout(LayoutKind.Auto)]
	internal readonly ref struct NewTextureResult {
		...
		internal readonly uint Scale { get; init; } = 0;
		...
		internal readonly IScalerInfo? ScalerInfo { get; init; } = null;
		internal readonly bool IsGradient { get; init; } = false;
	...
	internal static ulong GetHash(SpriteInfo input, TextureType textureType) {
		...
		if (SMConfig.Resample.EnableDynamicScale) {
	...
	private static unsafe NewTextureResult CreateNewTexture(
		...
		if (input.ReferenceData is null) {
			ThrowHelper.ThrowArgumentNullException(nameof(input.ReferenceData));
		}
		...
		if (SMConfig.Debug.Sprite.DumpReference) {
		...
		*/
		...
		else if (SMConfig.Resample.Analysis.BlockMultiple.Enabled) {
		...
		if (blockSize == Math.Max(inputBounds.Width, inputBounds.Height)) {
			if (!SMConfig.Resample.Recolor.Enabled) {
		...
		*/
		...
		bool isGradient = analysis.MaxChannelShades >= SMConfig.Resample.Analysis.MinimumGradientShades && (analysis.GradientDiagonal.Any || analysis.GradientAxial.Any);

		bool fullPremultiplyAlpha = true;
		bool premultiplyAlpha = SMConfig.Resample.PremultiplyAlpha;
		bool gammaCorrect = SMConfig.Resample.AssumeGammaCorrected;

		double opaqueProportion = (double)analysis.OpaqueCount / spriteRawExtent.Area;
		if (!isGradient) {
			if (opaqueProportion <= SMConfig.Resample.Analysis.MinimumPremultipliedOpaqueProportion) {
			...
			if (opaqueProportion >= SMConfig.Resample.Analysis.MaximumGradientOpaqueProportion) {
				isGradient = false;
			}
		}

		if (isGradient) {
			var blacklist = SMConfig.Resample.GradientBlacklistPatterns;
		...
		if (
			(
				!SMConfig.Resample.Recolor.Enabled &&
				!SMConfig.Resample.IsEnabled
			)||
		...
		bool resamplingAllowed = SMConfig.Resample.IsEnabled || CurrentConfiguredScaler == Scaler.None;

		if (SMConfig.Resample.EnableWrappedAddressing) {
		...
		// Apply recolor
		if (SMConfig.Resample.Recolor.Enabled) {
			float rScalar = (float)SMConfig.Resample.Recolor.RScalar;
			float gScalar = (float)SMConfig.Resample.Recolor.GScalar;
			float bScalar = (float)SMConfig.Resample.Recolor.BScalar;
		...
		if (scalerInfo is not null) {
			...
			if (SMConfig.Resample.Scale) {
				int preferredMaxDimension = SMConfig.PreferredMaxTextureDimension;
			...
			try {
				...
				if (SMConfig.Resample.Deposterization.PreEnabled) {
				...
				if (SMConfig.Resample.Deposterization.PostEnabled) {
					bitmapDataWide = Deposterize.Enhance(bitmapDataWide, scaledSize, doWrap);
				}

				if (SMConfig.Resample.UseColorEnhancement) {
		...
		{
			var scaledSizeClamped = scaledSize.Min(SMConfig.ClampDimension);
			if (scaledSize != scaledSizeClamped) {
				...
				Debug.Trace($"Sprite {textureName} requires rescaling");
		...
		*/
		...
		// TODO : ref optimize
		if (SMConfig.Resample.TrimExcessTransparency) {
		...
		if (SMConfig.Resample.BlockCompression.Enabled && scaledSize.MinOf >= 4 && (scalerInfo?.BlockCompress ?? true)) {
			...
			bool hasB = true;
			{
				...
				if (hasAlpha && !isPunchThroughAlpha) {
					...
					isMasky = alphaDeviation < SMConfig.Resample.BlockCompression.HardAlphaDeviationThreshold;
				}
			}
...
#pragma warning restore CS9091
	...
	private static ManagedTexture2D? UpscaleInternal(ManagedSpriteInstance spriteInstance, ref uint scale, SpriteInfo input, ulong hash, ref Vector2B wrapped, bool async, out ResampleStatus result) {
		...
		if (SMConfig.Garbage.CollectAccountUnownedTextures && GarbageMarkSet.Add(input.Reference)) {
		...
		try {
			...
			if (bitmapData.IsEmpty) {
				...
				try {
					var newTextureResult = CreateNewTexture(
						textureName: spriteInstance.NormalizedName(),
						async: async,
						input: input,
			...
			var isAsync = SMConfig.AsyncScaling.Enabled && async;
			if (!isAsync || SMConfig.AsyncScaling.ForceSynchronousStores || DrawState.ForceSynchronous) {
