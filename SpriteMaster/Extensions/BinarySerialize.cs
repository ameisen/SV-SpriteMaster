...
static class BinarySerialize {
	...
	internal static Action<BinaryWriter, object> GetWriter(this Type type) {
		return type switch {
			...
			var _ when type == typeof(InterlockedULong) => (stream, obj) => stream.Write((ulong)(InterlockedULong)obj),
	...
	internal static Func<BinaryReader, object> GetReader(this Type type) {
		return type switch {
			...
			var _ when type == typeof(InterlockedULong) => (stream) => stream.ReadUInt64(),
	...
	internal static uint GetSerializedSize(this Type type) {
		return type switch {
			...
			var _ when type == typeof(InterlockedULong) => sizeof(ulong),
